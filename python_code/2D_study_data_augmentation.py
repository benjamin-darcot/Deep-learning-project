...
#Function to split the data according to a split ratio
...
def get_data(img_w, img_h, number_of_layer_per_subj):
    ...
    for ind, mask in enumerate(mask_list):
        ...
        for layer in layers:
            ...
            layer_mask = resize(layer_mask, (img_h, img_w), anti_aliasing = True).astype('float32')#resize because not all have the same size
            ...
            if len(name_subj) == 1:
                ...
                adc_array = (adc_array - np.min(adc_array))/(np.max(adc_array)-np.min(adc_array))#normalization
        ...
        #Print the progression of the loading
...
#Load data
x_data, y_data = get_data(112, 112, 1)
x_train, x_test, y_train, y_test = train_test_data(x_data, y_data)
del x_data, y_data
...
clear_session()
clf = get_unet(img_ch, img_w, img_h, n_base, batchnormal =True, dropout = False)
clf.compile(loss=[dice_coef_loss], optimizer = Adam(lr = LR), metrics=[dice_coef, Precision(), Recall()])
clf.fit_generator(train_generator, epochs = n_epochs, validation_data=val_generator, steps_per_epoch=25, validation_steps=25)
clf.save(path + 'models/5_slices_data_augm')
del clf

clear_session()
clf = get_unet(img_ch, img_w, img_h, n_base, batchnormal =True, dropout = False)
clf.compile(loss=[dice_coef_loss], optimizer = Adam(lr = LR), metrics=[dice_coef, Precision(), Recall()])
clf_hist = clf.fit_generator(train_generator, epochs = n_epochs, validation_data=val_generator, steps_per_epoch=25, validation_steps=25)
